...
list<T,Allocator>::insert(const_iterator pos, T const&val) {
	...
	node* new_node = create_node_with_hint(pos.itr_curr,val);
...
list<T,Allocator>::insert(const_iterator pos, T && val) {
	node* new_node = create_node_with_hint(pos.itr_curr,std::move(val));
...
list<T,Allocator>::insert(const_iterator pos, size_t count, T const& val) {
	...
	base_node* curr;
	try {
		temp_head = create_node_with_hint(pos.itr_curr,val);
		curr = temp_head;	
		base_node* temp;	
		for (size_t i = 1; i < count; ++i) {
			base_node* new_node = create_node_with_hint(curr,curr,val);
			curr->next = new_node;
			curr = curr->next;
		}
	} catch (...) {
...
void list<T, Allocator>::push_front(T const& val) {
	auto new_node = create_node_with_hint(pre_head,val);
...
void list<T, Allocator>::push_front(T && val) {
	auto new_node = create_node_with_hint(pre_head,std::move(val));
	connect_nodes(new_node, pre_head->next);
	connect_nodes(pre_head, new_node);
	++n;
}
...
void list<T,Allocator>::push_back(T const& val) {
	auto new_node = create_node_with_hint(aft_tail,val);	
...
void list<T,Allocator>::push_back(T && val) {
	auto new_node = create_node_with_hint(aft_tail,std::move(val));	
	connect_nodes(aft_tail->prev, new_node);
	connect_nodes(new_node, aft_tail);
	++n;
}
...
list<T,Allocator>::emplace(const_iterator pos, Args && ...args) {
	auto new_node = create_node_with_hint(pos.itr_curr,std::forward<Args>(args)...);
...
void list<T,Allocator>::emplace_front(Args &&...args) {
		auto new_node = create_node_with_hint(pre_head,std::forward<Args>(args)...);
		connect_nodes(new_node, pre_head->next);
		connect_nodes(pre_head, new_node);
		++n;
}
...
void list<T,Allocator>::emplace_back(Args && ...args) {
	auto new_node = create_node_with_hint(aft_tail,std::forward<Args>(args)...);	
	connect_nodes(aft_tail->prev,new_node);
	connect_nodes(new_node, aft_tail);
}
	...
	if (count < new_size) {
		...
		try {
			while (count++ < new_size) {
				curr->next = create_node_with_hint(curr,curr,val);
				curr = curr->next;
				n++;
			}
		} catch (...) {
...
void list<T,Allocator>::assign(It first, It last) {
	...
	if (first != last) {
		...
		try {
			while (first != last) {
				new_curr->next = create_node_with_hint(new_curr,new_curr,*first);
	...
	if (other_curr != other.aft_tail) {
		...
		try {
			...
			while (other_curr != other.aft_tail) {
				new_curr->next = create_node_with_hint(new_curr,new_curr,static_cast<node*>(other_curr)->val);
				new_curr = new_curr->next;
				other_curr = other_curr->next;
			}
		} catch (...) {
...
list<T,Allocator>::operator=(list<T,Allocator> && other) {
	if (std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value) {
		...
		if (other_curr != other.aft_tail) {
			...
			base_node* new_curr;
			try {
				temp_head = create_node_with_hint(other_curr,std::move(static_cast<node*>(other_curr)->val));
				new_curr = temp_head;
				other_curr = other_curr->next;
				while (other_curr != other.aft_tail) {
					new_curr->next = create_node_with_hint(new_curr,new_curr,static_cast<node*>(other_curr)->val);
